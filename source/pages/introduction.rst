...
**Model Customization**
    ...
    - **Temporal resolution:** Each instance is built with a timeslice definition chosen for the question being asked, the time available to solve, and the size of the instance (regions × technologies). The catalog runs from 12 slices for screening and large models to 72 for questions that need the shape of the day. See :doc:`timeslice_definitions`.
